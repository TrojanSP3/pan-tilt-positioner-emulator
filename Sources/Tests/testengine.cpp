...
std::string PrintEngineStatus(Engine* engine)
{
    ...
    const size_t MAX_LENGTH_LINE_1 = std::max(WORD_AZIMUTH.length(),WORD_ELEVATION.length())+1;
    const size_t MAX_LENGTH_LINE_2 = (WORD_POSITION.length()+MAX_POSITION_LEN+1);
    const size_t MAX_LENGTH_LINE_3 = (WORD_SPEED.length()+MAX_SPEED_LEN+1);
    const size_t MAX_LENGTH = std::max(std::max(MAX_LENGTH_LINE_1,MAX_LENGTH_LINE_2),MAX_LENGTH_LINE_3);
...
void TestEngine::Infinity_Test_Mode()
{
    ...
    while(true)
    {
        ...
        while ((engine.GetAzimuthSpeed()) || (engine.GetElevationSpeed()))
        {
            ...
            TestFramework::LogMsg(PrintEngineStatus(&engine));
            int new_speed_azimuth= static_cast<int>(-percent*SPEED_AZIMUTH);
            int new_speed_elevation= static_cast<int>(-percent*SPEED_ELEVATION);
